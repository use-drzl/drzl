...
async function main() {
  try {
    ...
    if (bgArg) {
      ...
      const sample = { left: Math.round(w * 0.05), top: Math.round(h * 0.05), width: Math.max(4, Math.round(w * 0.2)), height: Math.max(4, Math.round(h * 0.2)) }
    ...
    await social.composite([{ input: markForSocial, gravity: 'center' }]).png().toFile(path.join(outPublic, 'social-card.png'))

    // Hero banner with gradient sky + globe grid and headline
    const bannerW = 1800, bannerH = 480
    const cx = bannerW / 2, cy = bannerH + 220
    ...
    const meridians = [-30, -18, 0, 18, 30].map((deg) => {
