/**
 * `generate` and `watch` hand the Express generator the same options, confirmed on the bytes.
 *
 ...
 * here call `expressOptions` instead of building the object themselves, and why this file checks
...
const ROOT = path.join(import.meta.dirname, '.express-parity-tmp');
...
/**
 * Every option the Express branch is supposed to forward, all set to something other than the
 * generator's own default, so a dropped one changes the output rather than coinciding with it.
 */
const CONFIG = `export default {
  schema: './src/db/schema.ts',
  outDir: './unused-by-express',
  generators: [
...
const expressOut = path.join(dir, 'out', 'express');
...
beforeAll(async () => {
  ...
  fromGenerate = await tree(expressOut);
  ...
  const child = spawn(
    process.execPath,
    [CLI, 'watch', '--pipeline', 'generate-express', '--poll'],
    {
      cwd: dir,
      stdio: 'ignore',
    }
...
describe('the Express branch in generate and in watch', () => {
  ...
  it('honoured every option the fixture set, so the comparison was not of two defaults', () => {
    ...
    expect(routes, 'importExtension: none').toContain("from './validation'");
    ...
    expect(routes, 'format.enabled: false').toContain("from 'express'");
    ...
    expect(barrel, 'the mounted segment').toContain("app.use('/users', users_api);");
  });
  ...
  it('leaves the top-level outDir alone when the generator names its own path', () => {
    ...
    // configured.
    expect(existsSync(path.join(dir, 'unused-by-express'))).toBe(false);
  });
});
