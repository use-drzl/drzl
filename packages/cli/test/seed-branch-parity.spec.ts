/**
 * `generate` and `watch` hand the seed generator the same options, confirmed on the bytes.
 *
 ...
 * `seedOptions` instead of building the object themselves.
 *
 * The fixture's schema carries a `CHECK`, which is the point of this generator: a branch that
 * dropped an option would still emit rows, and the byte comparison is what sees the difference.
 *
...
const ROOT = path.join(import.meta.dirname, '.seed-parity-tmp');
...
/**
 * Every option the seed branch forwards, set to something other than the generator's own default,
 * so a dropped one changes the output rather than coinciding with it.
 */
const CONFIG = `export default {
  schema: './src/db/schema.ts',
  outDir: './unused-by-seed',
  generators: [
    {
      kind: 'seed',
      path: './out/seed',
...
const seedOut = path.join(dir, 'out', 'seed');
...
beforeAll(async () => {
  ...
  fromGenerate = await tree(seedOut);
  ...
  const child = spawn(process.execPath, [CLI, 'watch', '--pipeline', 'generate-seed', '--poll'], {
    cwd: dir,
    stdio: 'ignore',
  });
  try {
...
describe('the seed branch in generate and in watch', () => {
  ...
  it('honoured every option the fixture set, so the comparison was not of two defaults', () => {
    const rows = fromGenerate['products_fixture.ts'];
    expect(rows, 'outputHeader.text').toContain('// parity fixture');
    expect(rows, 'count').toContain('count = 7');
    expect(rows, 'importExtension: none').toContain("from './rng'");
    // The CHECK reached the emitted values, which is the whole point of this generator.
    expect(rows, 'the parsed bound').toContain('rng.int(1, 999)');

    const barrel = fromGenerate['index.ts'];
    expect(barrel, 'naming.routerSuffix + procedureCase').toContain("from './users_fixture'");
    expect(barrel, 'the insert order').toContain('insertOrder');
  });

  it('leaves the top-level outDir alone when the generator names its own path', () => {
    expect(existsSync(path.join(dir, 'unused-by-seed'))).toBe(false);
  });
});
