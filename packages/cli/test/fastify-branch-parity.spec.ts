/**
 * `generate` and `watch` hand the Fastify generator the same options, confirmed on the bytes.
 *
 ...
 * here call `fastifyOptions` instead of building the object themselves, and why this file checks
...
const ROOT = path.join(import.meta.dirname, '.fastify-parity-tmp');
...
/**
 * Every option the Fastify branch is supposed to forward, all set to something other than the
 * generator's own default, so a dropped one changes the output rather than coinciding with it.
 * There is no `validation` entry, unlike the Hono and Express fixtures: this generator takes
 * none, and `resolveConfig` warns when a config sets one.
 */
const CONFIG = `export default {
  schema: './src/db/schema.ts',
  outDir: './unused-by-fastify',
  generators: [
    {
      kind: 'fastify',
      path: './out/fastify',
...
const fastifyOut = path.join(dir, 'out', 'fastify');
...
beforeAll(async () => {
  ...
  fromGenerate = await tree(fastifyOut);
  ...
  const child = spawn(
    process.execPath,
    [CLI, 'watch', '--pipeline', 'generate-fastify', '--poll'],
    {
      cwd: dir,
      stdio: 'ignore',
    }
...
describe('the Fastify branch in generate and in watch', () => {
  it('writes the same set of files', () => {
    expect(Object.keys(fromWatch)).toEqual(Object.keys(fromGenerate));
    // No middleware module, unlike the Express set: the schemas ARE the validation.
  ...
  it('honoured every option the fixture set, so the comparison was not of two defaults', () => {
    ...
    expect(routes, 'format.enabled: false').toContain("from 'fastify'");
    ...
    expect(barrel, 'the registered prefix').toContain(
      "app.register(users_api, { prefix: '/users' });"
    );
  });
  ...
  it('leaves the top-level outDir alone when the generator names its own path', () => {
    ...
    // configured.
    expect(existsSync(path.join(dir, 'unused-by-fastify'))).toBe(false);
  });
});
