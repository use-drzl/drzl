/**
 * `generate` and `watch` hand the Next.js generator the same options, confirmed on the bytes.
 *
 ...
 * here call `nextOptions` instead of building the object themselves, and why this file checks that
 * they did rather than trusting that they did.
 *
 * This generator has one piece of wiring no other kind has, and it is the one worth pinning here:
 * `nextOptions` derives `validation.importPath` from the sibling validation generator's own `path`
 * and forces `useShared`, because this generator emits no schemas of its own. A branch that skipped
 * the builder would emit actions importing from nowhere, and the fixture below puts the schemas
 * somewhere non-default so that a dropped derivation changes the bytes rather than coinciding.
 *
...
const ROOT = path.join(import.meta.dirname, '.next-parity-tmp');
...
/**
 * Every option the Next.js branch forwards, set to something other than the generator's own default,
...
const CONFIG = `export default {
  ...
  outDir: './unused-by-next',
  generators: [
    { kind: 'valibot', path: './out/schemas' },
    {
      kind: 'next',
      path: './out/next',
      naming: { routerSuffix: 'Form', procedureCase: 'snake' },
...
const nextOut = path.join(dir, 'out', 'next');
...
beforeAll(async () => {
  ...
  fromGenerate = await tree(nextOut);
  ...
  const child = spawn(process.execPath, [CLI, 'watch', '--pipeline', 'generate-next', '--poll'], {
    cwd: dir,
    stdio: 'ignore',
  });
  try {
...
describe('the Next.js branch in generate and in watch', () => {
  ...
  it('honoured every option the fixture set, so the comparison was not of two defaults', () => {
    const actions = fromGenerate['users_form.ts'];
    expect(actions, 'outputHeader.text').toContain('// parity fixture');
    expect(actions, 'validation.library').toContain("import * as v from 'valibot';");
    // format.enabled false, so nothing rewrote the quotes above and made those checks vacuous.
    expect(actions, 'the use server directive leads the file').toMatch(/^'use server';/);

    // The derivation this builder alone does: the schemas live at `./out/schemas`, which is not
    // the default for any library, and the actions module has to have found it from the sibling
    // entry rather than from anything written on this one.
    expect(actions, 'derived validation.importPath').toContain("from '../schemas/index'");

    const barrel = fromGenerate['index.ts'];
    expect(barrel, 'naming.routerSuffix + procedureCase').toContain("from './users_form'");
    expect(barrel, 'importExtension: none').toContain("export * from './posts_form';");
  });

  it('leaves the top-level outDir alone when the generator names its own path', () => {
    expect(existsSync(path.join(dir, 'unused-by-next'))).toBe(false);
  });
});
