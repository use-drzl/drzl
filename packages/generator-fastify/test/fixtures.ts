...
/**
 * The serializer's fixture, beyond what the shared family carries.
 *
 * This generator's response schemas drive fast-json-stringify, which silently omits, coerces and
 * truncates (see the measured grid in `src/index.ts`), so the round-trip proof needs the column
 * kinds with serializer-specific behaviour in one row: a `Date` that must come back as its ISO
 * string, a `bigint` that must come back as digits, a tuple whose 2020-12 `prefixItems` spelling
 * Fastify's draft-07 AJV refuses outright and this generator respells, an integer column, a
 * boolean, and a nullable column. A fixture family without these is how 13 defects shipped once.
 */
