...
describe('columns the database supplies', () => {
  ...
  it('treats a MySQL autoincrement column the same as a Postgres serial', async () => {
    // It used to be marked generated, which drops it from insert schemas entirely. That is the
    // wrong half of the distinction: AUTO_INCREMENT supplies a value when you omit one, it does
    // not forbid you from supplying your own, and `drizzle-orm/zod` makes it optional for exactly
    // that reason. The same construct also behaved differently per dialect, since a Postgres
    // `serial` was already treated as merely defaulted.
    const f = await flags(
      'gen-mysql-auto',
      `
    ...
    expect(f.a.hasDefault, 'the database will supply it').toBe(true);
    expect(f.a.isGenerated, 'but you may supply your own').toBe(false);
  });
});

describe('columns the database refuses a value for', () => {
  // The distinction that matters for an insert schema: not "who normally supplies this" but
  // "would the database reject a value if I sent one". Only these may be omitted outright.
  //
  // None of them were detected. The analyzer read `col.isGenerated`, which is undefined on every
  // Drizzle column of every dialect, so a `GENERATED ALWAYS AS` column appeared in insert schemas
  // and the insert built from one is rejected by Postgres.

  it('marks a stored generated column as generated', async () => {
    const f = await flags(
      'gen-pg-always',
      `
      import { pgTable, integer } from 'drizzle-orm/pg-core';
      import { sql } from 'drizzle-orm';
      export const a = pgTable('a', { total: integer('total').generatedAlwaysAs(sql\`1\`) });
      `
    );
    expect(f.a.isGenerated).toBe(true);
  });

  it('separates the two kinds of identity column', async () => {
    // GENERATED ALWAYS AS IDENTITY rejects an explicit value; GENERATED BY DEFAULT accepts one.
    // Official agrees: it omits the first and makes the second optional.
    const f = await flags(
      'gen-pg-identity-kinds',
      `
      import { pgTable, integer } from 'drizzle-orm/pg-core';
      export const a = pgTable('a', { id: integer('id').generatedAlwaysAsIdentity() });
      export const b = pgTable('b', { id: integer('id').generatedByDefaultAsIdentity() });
      `
    );
    expect(f.a.isGenerated, 'GENERATED ALWAYS').toBe(true);
    expect(f.b.isGenerated, 'GENERATED BY DEFAULT').toBe(false);
    expect(f.b.hasDefault, 'GENERATED BY DEFAULT still has one').toBe(true);
  });

  it('marks a MySQL generated column as generated', async () => {
    const f = await flags(
      'gen-mysql-always',
      `
      import { mysqlTable, int } from 'drizzle-orm/mysql-core';
      import { sql } from 'drizzle-orm';
      export const a = mysqlTable('a', { total: int('total').generatedAlwaysAs(sql\`1\`) });
      `
    );
    expect(f.a.isGenerated).toBe(true);
  });
