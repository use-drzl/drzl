...
- 6aa7581: The transpiled schema is cached in the project, not wherever jiti happened to land
...
- 866dbaa: Correct what `allowsNaN` says about MySQL, which was true of the wrong client path
...
- 0e295da: Type a v1 `bigint({ mode: 'string' })` column as the string the driver returns, so its generated
...
- 20af57e: An MSSQL `tinyint` is bounded where SQL Server bounds it, a Cockroach `bit(n)` stops being
...
- 9ae621e: A `numeric(p, s)` column is bounded by the width it declares rather than by what a JS number can
...
- fec7c87: `bit`, `geometry` and SingleStore's `vector` are described instead of returning `unknown`.
...
- a13c900: The pgvector family is described instead of coming back `unknown`.
