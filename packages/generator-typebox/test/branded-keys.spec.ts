/**
 * Branded primary and foreign keys, for typebox.
 *
...
async function emit(opts: Record<string, unknown>): Promise<Record<string, string>> {
  ...
  for (const t of TABLES) {
    out[t.tsName] = await fs.readFile(path.join(dir, `${t.tsName}.typebox.ts`), 'utf8');
  }
...
describe('what the emitted schema says', () => {
  it('brands nothing unless asked', async () => {
    ...
    expect(out.posts).not.toContain('drzlBrand');
  });
  ...
  it('brands a primary key with its own token', async () => {
    ...
    expect(field(out.posts, 'SelectpostsSchema', 'id')).toBe(
      "drzlBrand(Type.Integer(), 'posts.id')"
    );
  });
  ...
  it('can brand keys without branding foreign keys', async () => {
    ...
    expect(field(out.posts, 'SelectpostsSchema', 'authorId')).toBe('Type.Integer()');
  });
});
...
describe('what the emitted schema does', () => {
  ...
  async function load(branded: boolean) {
    ...
    for (const t of TABLES) {
      ...
      await fs.rename(path.join(dir, `${t.tsName}.typebox.ts`), file);
  ...
  it('parses exactly what it parsed before, and hands the value back unchanged', async () => {
    ...
    // TypeBox's brand is a cast, so the schema object itself is unchanged, not merely equivalent.
    expect(JSON.stringify(branded.posts.SelectpostsSchema)).toBe(
      JSON.stringify(plain.posts.SelectpostsSchema)
    );
    expect(Value.Check(branded.posts.SelectpostsSchema, row)).toBe(true);
    expect(Value.Check(branded.posts.SelectpostsSchema, { ...row, authorId: 'no' })).toBe(false);
  });
});

const PROOF = `
import type { SelectusersOutput, UsersId } from './branded/users.typebox.js';
import type { SelectpostsOutput, PostsId } from './branded/posts.typebox.js';
...
const PLAIN_CONTROL = `
import type { SelectusersOutput } from './plain/users.typebox.js';
import type { SelectpostsOutput } from './plain/posts.typebox.js';
...
beforeAll(async () => {
  ...
  await new TypeBoxGenerator(analysis()).generate({ outDir: path.join(dir, 'plain') } as never);
