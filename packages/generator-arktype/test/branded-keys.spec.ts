/**
 * Branded primary and foreign keys, for arktype.
 *
...
async function emit(opts: Record<string, unknown>): Promise<Record<string, string>> {
  ...
  for (const t of TABLES) {
    out[t.tsName] = await fs.readFile(path.join(dir, `${t.tsName}.arktype.ts`), 'utf8');
  }
...
describe('what the emitted schema says', () => {
  ...
  it('can brand keys without branding foreign keys', async () => {
    ...
    expect(field(out.posts, 'SelectpostsSchema', 'authorId')).toBe("'number.integer'");
  });
});
...
describe('what the emitted schema does', () => {
  ...
  async function load(branded: boolean) {
    ...
    for (const t of TABLES) {
      ...
      await fs.rename(path.join(dir, `${t.tsName}.arktype.ts`), file);
  ...
  it('parses exactly what it parsed before, and hands the value back unchanged', async () => {
    ...
    expect(branded.posts.SelectpostsSchema(row)).toEqual(plain.posts.SelectpostsSchema(row));
    expect(branded.posts.SelectpostsSchema(row).authorId).toBe(3);
    // ArkType answers a failure with an errors array rather than by throwing.
    expect(Array.isArray(branded.posts.SelectpostsSchema({ ...row, authorId: 'no' }))).toBe(true);
  });
});

const PROOF = `
import type { SelectusersOutput, UsersId } from './branded/users.arktype.js';
import type { SelectpostsOutput, PostsId } from './branded/posts.arktype.js';
...
const PLAIN_CONTROL = `
import type { SelectusersOutput } from './plain/users.arktype.js';
import type { SelectpostsOutput } from './plain/posts.arktype.js';
...
beforeAll(async () => {
  ...
  await new ArkTypeGenerator(analysis()).generate({ outDir: path.join(dir, 'plain') } as never);
