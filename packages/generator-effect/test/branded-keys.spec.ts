/**
 * Branded primary and foreign keys, for effect.
 *
...
async function emit(opts: Record<string, unknown>): Promise<Record<string, string>> {
  ...
  for (const t of TABLES) {
    out[t.tsName] = await fs.readFile(path.join(dir, `${t.tsName}.effect.ts`), 'utf8');
  }
...
describe('what the emitted schema says', () => {
  it('brands nothing unless asked', async () => {
    ...
    expect(out.posts).not.toContain('Schema.brand');
  });
  ...
  it('brands a primary key with its own token', async () => {
    ...
    expect(field(out.posts, 'SelectpostsSchema', 'id')).toBe(
      "Schema.Int.pipe(Schema.brand('posts.id'))"
    );
  });
  ...
  it('brands a foreign key with the token of the table it points at', async () => {
    ...
    expect(field(out.posts, 'SelectpostsSchema', 'authorId')).toBe(
      "Schema.Int.pipe(Schema.brand('users.id'))"
    );
  });

  it('leaves an ordinary column alone', async () => {
    const out = await emit({ branded: true });
    expect(field(out.posts, 'SelectpostsSchema', 'title')).not.toContain('Schema.brand');
  });

  it('puts the brand inside the nullable wrapper, not outside it', async () => {
    const out = await emit({ branded: true });
    expect(field(out.posts, 'SelectpostsSchema', 'editorId')).toBe(
      "Schema.NullOr(Schema.Int.pipe(Schema.brand('users.id')))"
    );
  });
  ...
  it('exports an alias for the key it owns and none for a key it borrows', async () => {
    ...
    expect(out.users).toContain(
      "export type UsersId = Schema.Schema.Type<typeof SelectusersSchema>['id'];"
    );
  ...
  it('can brand keys without branding foreign keys', async () => {
    ...
    expect(field(out.posts, 'SelectpostsSchema', 'authorId')).toBe('Schema.Int');
  });
});
...
describe('what the emitted schema does', () => {
  ...
  async function load(branded: boolean) {
    ...
    await new EffectGenerator(analysis()).generate({
      outDir: dir,
    ...
    for (const t of TABLES) {
      ...
      await fs.rename(path.join(dir, `${t.tsName}.effect.ts`), file);
  ...
  it('parses exactly what it parsed before, and hands the value back unchanged', async () => {
    ...
    expect(S.decodeUnknownSync(branded.posts.SelectpostsSchema)(row)).toEqual(
      S.decodeUnknownSync(plain.posts.SelectpostsSchema)(row)
    );
    const decoded = S.decodeUnknownSync(branded.posts.SelectpostsSchema)(row) as {
      authorId: number;
    };
    expect(decoded.authorId).toBe(3);
    expect(() =>
      S.decodeUnknownSync(branded.posts.SelectpostsSchema)({ ...row, authorId: 'no' })
    ).toThrow();
  });
});

const PROOF = `
import type { SelectusersOutput, UsersId } from './branded/users.effect.js';
import type { SelectpostsOutput, PostsId } from './branded/posts.effect.js';
...
const PLAIN_CONTROL = `
import type { SelectusersOutput } from './plain/users.effect.js';
import type { SelectpostsOutput } from './plain/posts.effect.js';
...
beforeAll(async () => {
  ...
  await new EffectGenerator(analysis()).generate({ outDir: path.join(dir, 'plain') } as never);
