...
/**
 * One column of every kind a form has an input for, which is what decides the reader per column.
 * The two Date columns are the point: one required, one nullable.
 */
export const profile = table('profile', {
  columns: [
    ...
    col('bio', 'string', { nullable: true }),
    col('age', 'number'),
    col('rating', 'number', { nullable: true }),
    col('active', 'boolean', { hasDefault: true }),
    col('bornOn', 'Date'),
    col('seenAt', 'Date', { nullable: true }),
    col('status', 'string', { enumValues: ['draft', 'live'] }),
  ],
  primaryKey: { columns: ['id'] },
});
