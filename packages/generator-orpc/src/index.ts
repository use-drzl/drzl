...
const template: ORPCTemplateHooks = {
  ...
  imports: (tables, ctx) => {
    ...
    const outDirResolved = outDir ? (path.isAbsolute(outDir) ? outDir : path.resolve(process.cwd(), outDir)) : path.resolve(process.cwd(), 'src/api');
    // servicesDir should be the directory where service files are located
    const servicesDirResolved = servicesDir ? (path.isAbsolute(servicesDir) ? servicesDir : path.resolve(process.cwd(), servicesDir)) : path.resolve(process.cwd(), servicesDirDefault);
