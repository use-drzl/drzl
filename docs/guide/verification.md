...
of publishable packages, so it went from 19 to 21 as [MCP](/generators/mcp) and
[Next.js](/generators/next) arrived; the documented-config count is a count of runnable blocks in
these docs rather than a measurement of the code, and it went from 36 to 38 as those two pages added
...
```
    ...
    node16 ok
    nodenext ok
    all 38 documented configs generate and typecheck
