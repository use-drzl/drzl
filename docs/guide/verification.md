...
of publishable packages, so it went from 19 to 26 as [MCP](/generators/mcp),
[Next.js](/generators/next), the [AI SDK](/generators/ai),
[TanStack Start](/generators/tanstack-start), [h3](/generators/h3),
[Effect HttpApi](/generators/effect-http) and [ts-rest](/generators/ts-rest) arrived; the
documented-config count is a count of runnable blocks in these docs rather than a measurement of the
code, and it went from 36 to 43 as those seven pages added one each. Both are updated in place rather than left stale, because this block is one of the three
...
```
    ...
    node16 ok
    nodenext ok
    all 43 documented configs generate and typecheck
