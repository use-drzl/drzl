...
of publishable packages, so it went from 19 to 32 as [MCP](/generators/mcp),
[Next.js](/generators/next), the [AI SDK](/generators/ai),
[TanStack Start](/generators/tanstack-start), [h3](/generators/h3),
[Effect HttpApi](/generators/effect-http), [ts-rest](/generators/ts-rest),
[Elysia](/generators/elysia), [seed data](/generators/seed),
[fast-check](/generators/fast-check), [openapi-fetch](/generators/openapi-fetch),
[forms](/generators/forms) and [Pothos](/generators/pothos) arrived; the documented-config count is
a count of runnable blocks in these docs rather than a measurement of the code, and it went from 36
to 49 as those thirteen pages added one each. Both are updated in place rather than left stale, because this block is one of the three
...
```
    ...
    node16 ok
    nodenext ok
    all 49 documented configs generate and typecheck
