...
of publishable packages, so it went from 19 to 25 as [MCP](/generators/mcp),
[Next.js](/generators/next), the [AI SDK](/generators/ai),
[TanStack Start](/generators/tanstack-start), [h3](/generators/h3) and
[Effect HttpApi](/generators/effect-http) arrived; the documented-config count is a count of runnable blocks in
these docs rather than a measurement of the code, and it went from 36 to 42 as those six pages added
...
```
    ...
    node16 ok
    nodenext ok
    all 42 documented configs generate and typecheck
