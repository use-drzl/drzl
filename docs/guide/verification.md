...
of publishable packages, so it went from 19 to 24 as [MCP](/generators/mcp),
[Next.js](/generators/next), the [AI SDK](/generators/ai),
[TanStack Start](/generators/tanstack-start) and [h3](/generators/h3) arrived; the documented-config count is a count of runnable blocks in
these docs rather than a measurement of the code, and it went from 36 to 41 as those five pages added
...
```
    ...
    node16 ok
    nodenext ok
    all 41 documented configs generate and typecheck
